...
function updateDockerfile(newVersion) {
  ...
  dockerfile = dockerfile.replace(
    /LABEL name="@waylit\/helpscout-mcp" \\\s*description="[^"]*" \\\s*version="[^"]*"/,
...
function createCommit(oldVersion, newVersion, bumpType) {
  try {
    // Stage the changes
    execSync(
      "git add package.json Dockerfile src/index.ts mcp.json server.json helpscout-mcp-extension/manifest.json",
    );
...
function main() {
  try {
    ...
    log(
      `  3. Merge to main for release: git checkout main && git merge dev && git push origin main --tags`,
    );
  } catch (error) {
